...
def apply_Eldar(problem_spec: ProblemSpec, prior_prob=None, min_prob=0):
    ...
    q = []
    for i in range(n):
        # q.append(np.outer(Phi_tilde[i], Phi_tilde[i]).round(1))
        q.append(np.outer(Phi_tilde[i], Phi_tilde[i]))
    q = np.array(q)
    ...
    result = prob.solve(solver=cp.SCS, verbose=True, max_iters=2_000_000)  #, eps=1e-20)
    ...
    for i in range(n):
        if sol[i] < 0:
            logger.warning(f"sol[{i}] is negative ({sol[i]}), skip its operator")
            continue
        else:
