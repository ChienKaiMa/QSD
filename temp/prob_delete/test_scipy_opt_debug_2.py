...
def test_idea():
    ...
    def mult_vec_and_state_real(states, num_basis, op_idx, x):
        ...
        # TODO Make the index look better
    ...
    x0 = [-0.5 for i in range(12)]
    x0 = [-0.5 for i in range(6)] + [0.5 for i in range(6)]
    def r0(x):
        return 0
    ...
    for op_idx in range(num_ops):
        ...
        for j in range(2**n - 2):
            basis = null_spaces[op_idx][:, j]
            print(basis)
            idx = op_idx * num_vars_per_op + 2 * j
            vec += basis * (x[idx] + x[idx + 1]*1j)
        print(vec)
