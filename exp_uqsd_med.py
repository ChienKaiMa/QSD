...
class QSD():
    ...
    def med_initialize(self, probability):
        ...
        dinominator = 1 - 2 * p2 * (inner ** 2) + np.sqrt(1 - 4 * p1 * p2 * (inner **2) )
        alpha = 2 * np.arctan(numerator / dinominator)
        # alpha = 2 * np.arctan2(numerator,dinominator)
...
def experiment(num_qubit, inner_product, method: str, initial_state: str,**karg):
    '''
    ...
    initial_state(str) - The input states are given by a pure state
    or mixed state. "Random" for mixed state. "Fixed" for pure state.
    ...
    **karg
    ...
    '''
    probability = karg.get("probability") if karg else 0.5
    ...
    v2 =  inner_product * np.exp(1j*rand_float)* v1 + np.sqrt(1-(inner_product**2)) *v1p
    v1_e = v1.copy()
    v2_e = v2.copy()
    ...
    for sub in qc_pre:
        ...
        qc.append(sub,list(range(qc_bit_num)))
        qc.barrier()
        qc.append(qc_iso,list(range(num_qubit)))
        qc.p(rand_float,0)
        qc.barrier()
        qc.append(qc_QSD, append_list)
        qc.measure_all()
        qc_total.append(qc)
...
method = "UQSD" # "UQSD" or "MED"
circuit_type = "random" # "Random" or "Fixed" incoming state.

# construct circuit
qc = experiment(num_qubit = num_qubit, inner_product = inner_product, method = method, initial_state = circuit_type, probability = probability)
# transpild circuit
...
#------ collect data -------#
if circuit_type in ["Fixed" , "fixed", "Fix", "fix"]:
    outcome = np.array([ np.array([probability, 1-probability])[i] 
                        * vec_to_array(result.quasi_dists[i].binary_probabilities(), num_qubit) 
                        for i in range(2)]).flatten()
else:
    outcome = vec_to_array(result.quasi_dists[0].binary_probabilities(), num_qubit+1)
...
# theoretical result
if method in ["MED", "med"]:
    print("MED success rate = ", (1 + np.sqrt(1-4* probability * (1-probability) * (inner_product)**2))/2)
else:
    print("UQSD success rate = ", 1-inner_product/2)
    
# experimental result
print("Experiment result : ", data_analysis(outcome,method))
